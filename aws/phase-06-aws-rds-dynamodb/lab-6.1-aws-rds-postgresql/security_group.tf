...
module "rds_postgresql_sg" {
  source  = "terraform-aws-modules/security-group/aws"
  version = "5.3.1"

  name        = "rds-postgresql-sg"
  description = "Allow PostgreSQL 5432 from web service SG and remote admin IP"
  vpc_id      = module.vpc.vpc_id
  ...
  ingress_with_source_security_group_id = [
    {
      ...
      to_port                  = 5432
      protocol                 = "tcp"
      description              = "PostgreSQL from web service SG"
  ...
  ingress_with_cidr_blocks = [
    {
      ...
      to_port     = 5432
      protocol    = "tcp"
      description = "PostgreSQL from remote admin"
